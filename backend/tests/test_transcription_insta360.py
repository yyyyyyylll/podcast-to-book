"""
单独测试 transcription 节点：影石Insta360 创始人刘靖康×罗永浩
播客链接: https://www.xiaoyuzhoufm.com/episode/694cb5e1946dca09253c1b2e

运行：cd backend && python tests/test_transcription_insta360.py
"""
import asyncio
import json
import sys
...
PODCAST_URL = "https://www.xiaoyuzhoufm.com/episode/694cb5e1946dca09253c1b2e"
...
def _log(msg: str):
    ...
    print(f"[insta360 {ts}] {msg}", flush=True)
...
async def step1_parse():
    ...
    _save(f"transcription_parse_insta360_{ts}.json", {
        "step": "parse",
        "url": PODCAST_URL,
...
async def step2_transcribe(episode):
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    _save(f"transcription_insta360_{ts}.json", {
        "step": "transcription",
        "elapsed_seconds": elapsed,
    ...
    md_lines = [
        "# Transcription 审查报告 — 影石Insta360 刘靖康×罗永浩",
        "",
    ...
    md_path = ARTIFACTS_DIR / f"transcription_review_insta360_{ts}.md"
...
async def main():
    _log("=" * 60)
    _log("Transcription 节点测试: 影石Insta360 刘靖康×罗永浩")
    _log("=" * 60)
